...
class AppConfig:
    ...
    def __init__(self):
        ...
        self.marker_sizes = {0: 120.0, 1: 30.0, 2: 120.0, 3: 30.0}
        self.camera_matrix = [[1800, 0, 960], [0, 1800, 540], [0, 0, 1]]
        self.dist_coeffs = [0, 0, 0, 0, 0]
        self.use_server_calibration = True
        ...
        if ids is not None:
            for i, mid in enumerate(ids.flatten()):
                if mid in self.config.marker_sizes:
                    ...
                    results[int(mid)] = {'rvec': rvec[0].tolist(), 'tvec': tvec[0].tolist(), 'quality': float(error)}
                    if self.config.show_axes:
                        ...
                        cv2.line(frame, corner, tuple(imgpts2[0].ravel()), (0,255,0), 3)
...
class NetworkClient:
    def __init__(self, config):
        self.config = config
        self.sio = socketio.Client(reconnection=config.auto_reconnect, reconnection_attempts=0,
    ...
    def setup_handlers(self):
        ...
        def on_connect():
            self.connected = True
            self.sio.emit('register_camera', {'camera_id': self.config.camera_id, 'capabilities': ['aruco_detection']})
    ...
    def send_markers(self, markers):
        if self.connected and self.config.send_to_server:
            self.sio.emit('marker_data', {'camera_id': self.config.camera_id, 'markers': markers, 'timestamp': time.time()})
...
class JawTrackerApp(App):
    def build(self):
        ...
        self.res_spinner = Spinner(text=f"{self.config.resolution[0]}x{self.config.resolution[1]}",
    ...
    def on_send_check(self, instance, value): self.config.send_to_server = value
    def on_axes_check(self, instance, value): self.config.show_axes = value
    def on_record_check(self, instance, value):
        self.config.record_video = value
        if value and self.tracking_active: self.start_recording()
    ...
    def toggle_tracking(self, instance):
        ...
        if self.tracking_active and self.config.record_video: self.start_recording()
        elif not self.tracking_active: self.stop_recording()
